...
func (p *blockBodyDiffPrinter) writeBlockBodyDiff(schema *configschema.Block, old, new cty.Value, indent int) {
...

	{
		...
		for _, name := range blockTypeNames {
			...
			p.writeNestedBlockDiffs(name, blockS, oldVal, newVal, blankBeforeBlocks, indent)
...
func (p *blockBodyDiffPrinter) writeNestedBlockDiffs(name string, blockS *configschema.NestedBlock, old, new cty.Value, blankBefore bool, indent int) {
	...
	case configschema.NestingSingle:
		...
		p.writeNestedBlockDiff(name, nil, &blockS.Block, action, old, new, indent)
	...
	case configschema.NestingSet:
		...
		for it := all.ElementIterator(); it.Next(); {
			...
			p.writeNestedBlockDiff(name, nil, &blockS.Block, action, oldValue, newValue, indent)
		}
...
func (p *blockBodyDiffPrinter) writeNestedBlockDiff(name string, label *string, blockS *configschema.Block, action diffs.Action, old, new cty.Value, indent int) {
	p.buf.WriteString(strings.Repeat(" ", indent))
	p.writeActionSymbol(action)
	...
	p.writeBlockBodyDiff(blockS, old, new, indent+4)
